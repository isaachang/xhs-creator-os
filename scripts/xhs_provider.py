...
def media_root() -> Path:
    configured = os.environ.get("MEDIA_CRAWLER_ROOT", "").strip()
...
def media_installed(root: Path | None = None) -> bool:
    ...
    return (root / "main.py").is_file() and media_python(root).is_file()
...
def run_media(command: list[str], *, root: Path) -> None:
