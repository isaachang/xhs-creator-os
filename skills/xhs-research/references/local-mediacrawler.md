...
它使用独立、被 Git 忽略的小红书专用 Profile，适合作为低频本机验证；不会连接用户日常 Chrome。首次启动后会根据专用 Profile 的会话域名、实际页面跳转和扫码后的最终页面自动识别 `xiaohongshu.com` 或 `rednote.com`。只保存站点名称，不保存或输出 Cookie；后续复用同一专用 Profile，不要求用户手动选择站点。正常抓取默认后台运行；登录失效时仅允许一次二维码登录会话，二维码由 Codex 展示，不能自动循环重试或额外打开用户浏览器。原上游仓库为非商业学习许可，不作为公开分发 Skill 的默认数据源。

## 登录会话与作者 Top-N

- 使用 `python3 scripts/xhs_provider.py media-auth --qr-output <临时二维码路径>` 校验专用会话。若失效，等待用户扫码后由同一进程继续；二维码文件不含 Cookie，成功后可删除。
- Agent 启动 `media-auth` 后应短暂轮询二维码路径；文件出现即在 Codex 展示图片，并保持原登录进程等待。用户回复“已扫码”后只能轮询该原进程的结果，不能再启动第二个登录任务。
...
- 后台模式被安全校验拦截时，明确提示用户改用 `--media-browser visible` 进行一次专属可见会话验证；不得无提示降级或重复扫码。
