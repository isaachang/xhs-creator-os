...
After every prompt file for the current generation group has been saved and verified, generate images in batches by default. Treat the cover as generation group 1 and images 2+ as generation group 2; the cover-first quality gate between the groups still applies.
...
- Never include the cover and later cards in the same batch.
...
When this skill is called by the project-level `xhs-image` adapter, the adapter owns the user-facing planning gate and phase state. Baoyu must honor the adapter's `confirmation_mode` and `phase` values; it must not infer that a request to generate images also authorizes skipping confirmation. If the adapter has not supplied an explicit direct-generation signal, use the default confirmation path.

- Treat explicit skill invocation, a file path, matched signals/presets, and `EXTEND.md` defaults as recommendation inputs only.
- In the default path, complete Step 2, present the plan, and wait for the user's selection/approval before rendering.
- An explicit direct-generation intent such as `--yes`, "直接生成一套", "直接出图", "不用确认方案", "跳过确认", or equivalent enables **direct mode**. Direct mode may auto-confirm the recommended plan and continue without waiting at user-confirmation gates.
- Direct mode must still save the analysis, outline, and prompt files, generate the cover before using it as the series anchor, and run all image-quality checks. Skipping user confirmation never means skipping quality review.
- State the assumed strategy / style / layout / palette / count / backend in the next user-facing update when direct mode is used.
...
**Cover-first generation order — hard**: Always generate the cover before rendering later cards so it can become the series anchor. In the default path, pause after the cover for the remaining-card approval. In direct mode, do not pause for user approval, but still require the cover to pass the image-quality review before using it as the anchor.

Generation flow:

1. Finalize and write only the cover prompt to `prompts/01-cover-{slug}.md` in the user's preferred language (backup rule applies). Do not finalize or save prompts for images 2+ yet.
2. Generate **image 1** without `--ref`; backup rule applies to the PNG file. This establishes the anchor.
3. Inspect the cover for exact readable text, structural quality, composition, and style. Regenerate the cover from a revised saved prompt until it passes; do not advance with a flawed cover.
4. After the cover passes, send the user a text section titled `后续卡片生图方案` in the default path. For every proposed image 2+ state:
   - page number, page purpose, and exact on-image copy;
   - layout, scene, and key visual elements;
   - the IP expression/emotion to use;
   - the exact expression reference filename when an IP routing file exists;
   - one brief reason the emotion fits that page.
5. In the default path, **stop and wait for explicit user approval**. In direct mode, continue automatically after recording the assumed plan; do not wait for another user response.
6. After approval, write and verify every full prompt for images 2+ under `prompts/NN-{type}-{slug}.md`.
7. Build a task list for **images 2+** using the approved cover as `--ref <path-to-image-01.png>`.
8. Dispatch images 2+ in batches per the `## Batch Generation Policy`: backend native batch first, runtime parallel tool calls second, sequential only as fallback.
9. Report progress after each completed image. On failure, retry only the failed item once from the same saved prompt file.
